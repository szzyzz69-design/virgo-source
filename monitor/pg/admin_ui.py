...
def _build_table_panel(ui: Any, service: PgAdminService, table_name: str) -> None:
            ...
            table.add_slot(
                ...
                <q-td :props="props">
                    <q-btn flat dense no-caps
                        ...
                        <q-popup-proxy><q-card class="q-pa-md text-body2">{{ props.value }}</q-card></q-popup-proxy>
                    </q-btn>
                </q-td>
                """,
            )
    ...
    with ui.row().classes("items-center gap-2"):
        ...
        if table_name == "devices":
            ui.button(
                icon="edit",
                on_click=lambda: _with_selected(ui, table, lambda row: _open_device_dialog(ui, service, refresh, row)),
            ).tooltip("编辑设备名称")
            ui.button(
                icon="delete",
                color="negative",
...
def _open_sim_dialog(
    ...
    with ui.dialog() as dialog, ui.card().classes("w-full max-w-xl gap-3"):
        ...
        enabled = ui.select({True: "已启用", False: "未启用"}, label="状态", value=bool(row.get("enabled_raw", row.get("enabled", True)))).props("outlined dense").classes("w-full")
        areas = ui.select(
...
def _open_device_dialog(ui: Any, service: PgAdminService, refresh: Any, row: dict[str, Any]) -> None:
    with ui.dialog() as dialog, ui.card().classes("w-full max-w-xl gap-3"):
...
def _parse_date_input(value: str | None) -> int | None:
    if not value:
        return None
    moment = datetime.combine(datetime.strptime(value, "%Y-%m-%d").date(), datetime_time.min)
