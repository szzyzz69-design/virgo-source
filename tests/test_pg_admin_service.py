...
def test_create_product_requires_an_account():
    database = RecordingDatabase()
    service = PgAdminService(database)

    try:
        service.create_product(ProductCreate(id="prod_1", menu="menu", update_by=None))
    except ValueError as error:
        assert str(error) == "Product account is required"
    else:
        raise AssertionError("product without account was accepted")
...
def test_update_product_keeps_id_and_refreshes_update_time():
    ...
    service.update_product(
        "prod_1",
        ProductUpdate(menu="new reminder", update_by="acc_2", areas="US"),
    )
    ...
    assert database.params[0] == ("new reminder", 234567, "acc_2", "US", "prod_1")


def test_update_device_changes_only_admin_name_and_timestamp():
    database = RecordingDatabase()
    service = PgAdminService(database, now_ms=lambda: 456789)

    service.update_device("dev_1", DeviceUpdate(name="Calgary front desk"))

    assert database.statements[0] == "UPDATE devices SET name = %s, updated_at = %s WHERE id = %s"
    assert database.params[0] == ("Calgary front desk", 456789, "dev_1")


def test_update_product_requires_an_account():
    database = RecordingDatabase()
    service = PgAdminService(database)
    ...
    try:
        service.update_product(
            "prod_1",
            ProductUpdate(menu="new reminder", update_by="  ", areas="US"),
        )
    except ValueError as error:
        assert str(error) == "Product account is required"
    else:
...
def test_list_contact_rows_searches_by_phone_suffix_before_limiting_results():
    database = RecordingDatabase(
        rows=[
            {
                "id": "contact_1",
    ...
    rows = service.list_rows("contacts", phone_query=" 12-34 ")
    ...
    assert "status <> 'ARCHIVED'" in database.statements[0]
    assert "regexp_replace" in database.statements[0]
    assert "LIKE %s" in database.statements[0]
    assert database.params[0] == ("%1234%", 200)


def test_list_contact_rows_with_non_phone_query_returns_no_results():
    database = RecordingDatabase(rows=[{"id": "contact_1"}])
    service = PgAdminService(database)

    rows = service.list_rows("contacts", phone_query="not a phone")

    assert rows == []
    assert database.statements == []


def test_phone_search_digits_accepts_formatted_phone_or_suffix():
    assert _phone_search_digits("+86 (138) 0000-1234") == "8613800001234"
    assert _phone_search_digits("1234") == "1234"
    assert _phone_search_digits("  ") is None
