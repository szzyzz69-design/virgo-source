...
def test_agent_menu_list_isolated_by_account_even_when_accounts_share_area(
    clean_database,
):
    username = "menu_contact_" + uuid4().hex
    other_username = "menu_contact_other_" + uuid4().hex
    password = "correct-password"
    area = "shared_" + uuid4().hex[:12]
    other_area = "south_" + uuid4().hex[:12]
    own_id = "prod_" + uuid4().hex
    own_other_area_id = "prod_" + uuid4().hex
    other_account_id = "prod_" + uuid4().hex
    blank_id = "prod_" + uuid4().hex
    with psycopg.connect(clean_database.dsn) as connection:
        account_id = _insert_account(connection, username, password, area)
        second_account_id = _insert_account(
            connection,
            other_username,
            password,
            area,
        ...
        connection.execute(
            ...
            VALUES(%s, %s, %s, %s, %s)
            """,
            (own_id, "own script", 1_800_000_000_003, account_id, area),
        )
        connection.execute(
            """
            INSERT INTO products(id, menu, update_time, update_by, areas)
        ...
        connection.execute(
            ...
            VALUES(%s, %s, %s, %s, %s)
            """,
            (blank_id, "   ", 1_800_000_000_001, account_id, area),
        )
        connection.commit()
    ...
    assert response.json() == [
        {
            "id": own_other_area_id,
            "menu": "own script with legacy area",
            "updateTime": 1_800_000_000_004,
            "updateBy": account_id,
            "areas": other_area,
        },
        {
