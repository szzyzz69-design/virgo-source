...
def _phone_search_digits(value: str | None) -> str | None:
...
class PgAdminService:
    ...
    def list_rows(
        ...
        if table_name == "contacts":
            return self._list_contacts(safe_limit, phone_query)
    ...
    def create_product(self, data: ProductCreate) -> None:
        account_id = _blank_to_none(data.update_by)
        if account_id is None:
            raise ValueError("Product account is required")
        self._execute(
            """
    ...
    def _list_contacts(
        ...
        columns = ", ".join(table.columns)
        phone_digits = _phone_search_digits(phone_query)
        if phone_digits == "":
            return []
        phone_filter = ""
        ...
        if phone_digits is not None:
            phone_filter = (
                "AND regexp_replace("
                "COALESCE(normalized_phone_number, phone_number), "
                "'[^0-9]', '', 'g') LIKE %s "
            )
            params = (f"%{phone_digits}%", limit)
        statement = (
    ...
    def list_sim_card_history(self, limit: int = 1000) -> list[dict[str, Any]]:
        with self._database.transaction() as connection:
            with connection.cursor(row_factory=dict_row) as cursor:
                cursor.execute('''SELECT id,sim_card_id,device_id,device_name,phone_number,
                    replacement_phone,areas,reason,occurred_at FROM sim_card_history
                    ORDER BY occurred_at DESC,id DESC LIMIT %s''',(max(1,min(limit,5000)),))
                return [dict(row) for row in cursor.fetchall()]

    def delete_sim_card(self, sim_card_id: str) -> None:
        self._execute('DELETE FROM sim_cards WHERE id = %s',(sim_card_id,))
