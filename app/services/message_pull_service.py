...
class MessagePullService:
    ...
    def pull(self, device_id: str, order: str) -> list[MessagePullItem]:
        ...
        with self._database.transaction() as connection:
            ...
            check_items = self._sms_checks.pull(connection, device_id, now, self.LIMIT - len(rows)) if self._sms_checks else []
            if not rows:
                return check_items
            ...
            return [self._to_item(row, recipients[row[0]]) for row in rows] + check_items
